...
test("test calculate", () => {
  ...
  expect(callback.mock.calls.lenghth).toBe(2);

  console.info(callback.mock.calls);

  expect(callback.mock.calls[0][0]).toBe(30);
  expect(callback.mock.calls[0][0]).toBe(50);
});
...
test("test mock return value", () => {
  ...
  callback.mockReturnValueOnce(90);
...
test("test mock implementation", () => {
  ...
  expect(calculateAndReturn[(10, 10, 10)], callback).toBe(100);
